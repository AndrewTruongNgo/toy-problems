 ...
 * we can calculate the index of a nodes parent or children using math:
 *
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  arr[0] = elem;
  let curr = 0;
  let leftNode = (curr * 2) + 1;
  let rightNode = (curr * 2) + 2;
  while(arr[curr] > arr[leftNode] || arr[curr] > arr[rightNode]) {
    if (arr[leftNode] > arr[rightNode]) {
      let temp = arr[curr];
      arr[curr] = arr[rightNode];
      arr[rightNode] = temp;
      curr = rightNode;
      leftNode = (curr * 2) + 1;
      rightNode = (curr * 2) + 2;
